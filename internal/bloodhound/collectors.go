...
func (s *Service) CollectorDownload(ctx context.Context, collector, releaseTag string) (string, string, error) {
	...
	if releaseTag == "" {
		releaseTag, err = latestCollectorTag(ctx, client, ctype)
		if err != nil {
			return "", "", err
		}
	}
	...
	checksum, err := fetchCollector(ctx, client, ctype, collector, releaseTag, dir, target)
	if err != nil {
		return "", "", err
	}
...
func latestCollectorTag(ctx context.Context, client *bh.Client, ctype bhservices.EnumClientType) (string, error) {
...
func fetchCollector(ctx context.Context, client *bh.Client, ctype bhservices.EnumClientType, collector, releaseTag, dir, target string) (string, error) {
